...
module.exports = new Object({
    ...
    async execute(client, dispatcher, track) {
        ...
        if (data && data.channel) {
            ...
            if (channel.id === textChannel.id) {
                if (data2 && data2.mode && data2.channel) {
                    ...
                    if (data2.prunning) {
                        if (channel && channel.sendable && channel.id !== textChannel.id) me1 = await channel.send({ embeds: [client.embed().setTitle('<a:vr_ga_spotify:1045284786345885696> Started Playing').setDescription(`[${track.title}](${track.uri}) by ${author}`).setColor(String(color))] }).then(x => dispatcher.data.set("message", x));
                    } else {
        ...
        if (data2 && data2.prunning) {
            await channel.send({ embeds: [client.embed().setTitle('<a:vr_ga_spotify:1045284786345885696> Started Playing').setDescription(`[${track.title}](${track.uri}) by ${author}`).setColor(String(color))] }).then(x => dispatcher.data.set("message", x));
        }
        await channel.send({ embeds: [client.embed().setTitle('<a:vr_ga_spotify:1045284786345885696> Started Playing').setDescription(`[${track.title}](${track.uri}) by ${author}`).setColor(String(color))] }).then(x => dispatcher.data.set("message", x));
    },
});